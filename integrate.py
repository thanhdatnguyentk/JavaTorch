...
for root, dirs, files in os.walk(path):
    for f in files:
        if f.startswith("Train") and f.endswith(".java") and "ResNet" not in f:
            ...
            print(f"Adding Dashboard setup to {f}")
...
"""
            ...
            # Let's replace any System.out.printf inside the loop that seems like an epoch log
            content = re.sub(
                r'((\s*)System\.out\.printf\(".*?Epoch.*?%n".*?\);)',
                r'\1\2try {\n' +
                r'\2    Map<String, Float> metrics = new HashMap<>();\n' +
                r'\2    // Add faux generic values to ensure graph updates if exact variables are un-mappable\n' +
                r'\2    metrics.put("epoch", (float)epoch);\n' +
                r'\2    history.record(epoch, metrics);\n' +
                r'\2    dashboard.broadcastMetrics(epoch, metrics);\n' +
                r'\2} catch (Exception dashEx) {}\n',
                content
            )

            # Shutdown outside loop
            content = re.sub(r'(System\.out\.println\("\\nTraining Complete!"\);)', r'\1\n        try { dashboard.exportDashboardData("dashboard_final.json"); } catch(Exception e) {}', content)
