...
public class TrainFashionMNIST {
    ...
    public static void main(String[] args) throws Exception {
        ...
        // Layer 1: 1 -> 32 channels, 3x3 kernel, pad 1 => 28x28
        model.add(new Conv2d(1, 32, 3, 3, 1, 1, 1, 1, true));
        model.add(new ReLU());
        // Pool: 28x28 -> 14x14
        model.add(new MaxPool2d(2, 2, 2, 2, 0, 0, 32, 28, 28));

        // Layer 2: 32 -> 64 channels, 3x3 kernel, pad 1 => 14x14
        model.add(new Conv2d(32, 64, 3, 3, 1, 1, 1, 1, true));
        model.add(new ReLU());
        // Pool: 14x14 -> 7x7
        model.add(new MaxPool2d(2, 2, 2, 2, 0, 0, 64, 14, 14));

        // Layer 3: 64 -> 128 channels, 3x3 kernel, pad 1 => 7x7
        ...
        // Fully Connected
        model.add(new Linear(6272, 512, true));
        model.add(new ReLU());
        model.add(new Dropout(0.3f));
        model.add(new Linear(512, 10, true));
        ...
        Data.Dataset trainDataset = new Data.Dataset() {
            ...
            public Tensor[] get(int index) {
                // Preprocess: Reshape to (1, 28, 28)
                float[] raw = trainImages[index];
                Tensor x = Torch.tensor(raw, 1, 28, 28);
        ...
        for (int epoch = 0; epoch < epochs; epoch++) {
            ...
            Data.Dataset testDataset = new Data.Dataset() {
                ...
                public Tensor[] get(int index) {
                    ...
                    return new Tensor[] { Torch.tensor(raw, 1, 28, 28), Torch.tensor(new float[] { testLabels[index] }, 1) };
                }
            };
            ...
            for (int ti = 0; ti < Math.min(200, testImages.length); ti++) {
                try (MemoryScope evalScope = new MemoryScope()) {
                    float[] raw = testImages[ti];
                    Tensor tx = Torch.tensor(raw, 1, 28, 28); tx.toGPU();
                    ...
                    if (livePreds.size() < 9) {
                        ...
                        livePreds.add(DashboardIntegrationHelper.buildLivePrediction(
                            DashboardIntegrationHelper.encodePixelsToBase64(raw, 1, 28, 28),
