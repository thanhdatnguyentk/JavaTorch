...
public class App {
    public static void main(String[] args) throws Exception {
        ...
        NN.Mat X = lib.mat_alloc(4, 3); // inputs with bias as last column
        X.es[0] = 0; X.es[1] = 0; X.es[2] = 1;
        X.es[3] = 0; X.es[4] = 1; X.es[5] = 1;
        X.es[6] = 1; X.es[7] = 0; X.es[8] = 1;
        X.es[9] = 1; X.es[10] = 1; X.es[11] = 1;

        NN.Mat Y = lib.mat_alloc(4, 1);
        Y.es[0] = 0; Y.es[1] = 1; Y.es[2] = 1; Y.es[3] = 0;

        // Network: 3 -> 2 -> 1 (includes bias in input)
        NN.Mat W1 = lib.mat_alloc(3, 2);
        NN.Mat W2 = lib.mat_alloc(2, 1);
        lib.mat_rand(W1, -1f, 1f);
        lib.mat_rand(W2, -1f, 1f);

        float lr = 0.5f;
        int epochs = 100000;

        NN.Mat hidden = lib.mat_alloc(4, 2);
        NN.Mat output = lib.mat_alloc(4, 1);

        for (int e = 0; e < epochs; e++) {
            // forward: hidden = sigmoid(X * W1)
            lib.mat_dot(hidden, X, W1);
            applySigmoid(hidden);

            // forward: output = sigmoid(hidden * W2)
            lib.mat_dot(output, hidden, W2);
            applySigmoid(output);

            // compute output error: delta_out = (output - Y) * output*(1-output)
            NN.Mat delta_out = lib.mat_alloc(output.rows, output.cols);
            // copy output into delta_out
            for (int i = 0; i < output.rows * output.cols; i++) delta_out.es[i] = output.es[i];
            lib.mat_sub(delta_out, Y); // delta_out = output - Y
            ...
            NN.Mat hidden_t = transpose(lib, hidden);
            NN.Mat gradW2 = lib.mat_alloc(hidden_t.rows, delta_out.cols);
            lib.mat_dot(gradW2, hidden_t, delta_out);
            ...
            NN.Mat delta_hidden = lib.mat_alloc(delta_out.rows, W2_t.cols);
            lib.mat_dot(delta_hidden, delta_out, W2_t);
