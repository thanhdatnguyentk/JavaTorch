...
def main():
    ...
    run_id = f"fashion_mnist_pytorch_{datetime.now().strftime('%Y%m%d_%H%M%S')}_{args.device}"
    os.makedirs(os.path.join(args.outputDir, "pytorch", "fashion_mnist", run_id), exist_ok=True)
    ...
    for epoch in range(args.epochs):
        ...
        for b, (inputs, targets) in enumerate(trainloader):
            ...
            if (b + 1) == 1 or (b + 1) % 50 == 0 or (b + 1) == len(trainloader):
                print(f"[Benchmark][FashionMNIST][Train] epoch={epoch+1}/{args.epochs} batch={b+1}/{len(trainloader)} loss={loss.item():.5f}")
        ...
        print(f"[Benchmark][FashionMNIST] epoch={epoch+1}/{args.epochs} loss={avg_loss:.5f} train_acc={train_acc:.4f} val_acc={val_acc:.4f} time_ms={epoch_ms}")
