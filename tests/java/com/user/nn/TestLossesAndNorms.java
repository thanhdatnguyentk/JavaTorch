...
public class TestLossesAndNorms {
    public static void main(String[] args) {
        ...
        for (int j = 0; j < out.cols; j++) {
            float s = 0f;
            for (int i = 0; i < out.rows; i++) s += out.es[i*out.cols + j];
            float mean = s / out.rows;
            if (Math.abs(mean) > 1e-5f) { System.err.println("BatchNorm mean not zero"); System.exit(3); }
        }

        System.out.println("TEST PASSED: LossesAndNorms");
    }
}
